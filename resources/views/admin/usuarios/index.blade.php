        ...
        <div class="card-body">
            ...
            <div class="table-responsive pt-3">
                ...
                <table class="table table-striped">
                    ...
                    <tbody>
                        @foreach($listagem['result'] as $usuario)
                        <tr>
                            <th scope="row">{{ $usuario->id }}</th>
                            <td>{{ $usuario->name }}</td>
                            <td>{{ $usuario->email }}</td>
                            <td>
                                <button class="btn btn-primary" style="padding:5px 10px" onclick="edit({{ $usuario->id }})">
                                    <i class="fa fa-pencil-square" aria-hidden="true"></i>
                                </button>
                                <button class="btn btn-danger" style="padding:5px 10px" onclick="del({{ $usuario->id }})">
...
<script>
    ...
    async function edit(id) {
        ...
        let dados = await $.ajax({
                type: "GET",
                dataType: "json",
                url: base_url + 'admin/usuarios/'+id+'/edit'
            });
    ...
    window.onload = function() {
        ...
        $('#btn-cadastrar').click(() => {
            ...
            let campoSenha = `<div class="form-group col-md-6 field-password">
                                <label for="password">Password</label>
                                <input type="password" class="form-control" id="password" name="password" placeholder="Password">
                            </div>`;

            $('#id').val("");
            $("#formCadastro .form-row").append(campoSenha);
        ...
        $('#btn-salvar').click(() => {
            ...
            let restUrl = base_url + 'admin/usuarios';
            let _method =  'POST';
            ...
            if($('#id').val() !== '') {
                ...
                restUrl = base_url + 'admin/usuarios/'+ $('#id').val();
                console.log(dados);
            }
